...
class VulnerabilityAnalyzer:
    ...
    def _get_model(self):
        ...
        if os.path.exists(self.model_path):
            ...
            m = LitGNN.load_from_checkpoint(self.model_path, map_location=self.device)
            m.to(self.device)
            m.eval()
            return m
        else:
    ...
    def _train_model(self):
        ...
        accel    = type(trainer.accelerator).__name__
        strategy = type(trainer.strategy).__name__
        root_dev = getattr(trainer.strategy, "root_device", None)
        dev_ids  = getattr(trainer, "device_ids", None)

        print(f">>> Accelerator: {accel}")
        print(f">>> Strategy:    {strategy}")
        print(f">>> Root device: {root_dev}")
        print(f">>> Device IDs:  {dev_ids}")
    ...
    def _prepare_test_data(self):
        ...
        for col in ("dataset", "source", "corpus", "project", "repo"):
            if col in df.columns and df[col].nunique() > 0:
                ...
                break
        if ds_name is None:
            # fallback: use the "processed" folder name

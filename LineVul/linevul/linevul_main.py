...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    if args.write_raw_preds:
        write_raw_preds_csv(args, y_preds, logits[:, 1])
    ...
    if args.do_local_explanation:
        for reasoning_method in all_reasoning_method:
                ...
                if index in tp_indices:
                    ...
                    if isinstance(df["flaw_line"][index], str) and isinstance(df["flaw_line_index"][index], str):                        
                        ...
                        line_level_localization_tp(flaw_lines=df["flaw_line"][index],
                                                ...
                                                all_line_scores=all_line_scores)
            ...
            if args.save_line_scores:
                os.makedirs("./results", exist_ok=True)
                df = pd.DataFrame(all_line_scores)
                df.to_csv("./results/line_scores_attention.csv", index=False)
                logger.info("Saved line-level attention scores to ./results/line_scores_attention.csv")
...
def write_raw_preds_csv(args, y_preds, confidences):
    df = pd.read_csv(args.test_data_file)
...
def line_level_localization_tp(flaw_lines: str, tokenizer, model, mini_batch, original_func: str, args, top_k_loc: list, top_k_constant: list, reasoning_method: str, index: int, write_invalid_data: bool, all_line_scores):
    ...
    if do_explanation:
        ...
        if args.save_line_scores:
            for i, score in enumerate(all_lines_score):
                ...
                all_line_scores.append({
                    ...
                    "line_index": i,
                    "score": score,
                    "is_flaw": 1 if i in flaw_line_indices else 0
                })
        return results
    else:
...
def main():
    ...
    parser.add_argument("--save_line_scores", default=False, action='store_true',
                        help="Save a global CSV with line-level scores from attention.")

...
# Evaluate routing accuracy and per-class and misclassification counts
def evaluate_routing(predictor: MultiModalPredictor, test_data_path: str) -> None:
    # Load test data
    df = pd.read_csv(test_data_path)
    if 'class_CWE' not in df.columns:
        raise KeyError("Dataset must contain a 'class_CWE' column with ground truth labels.")
    ...
    total = len(df)
    correct = (df['predicted_class'] == df['class_CWE']).sum()
    ...
    correct_df = df[df['predicted_class'] == df['class_CWE']]
    per_class = (
        correct_df
        .groupby('class_CWE')
        .size()
        .reset_index(name='correct_count')
        .sort_values('class_CWE')
    )
    print("\n=== Correct Predictions per CWE ===")
    print(per_class.to_string(index=False))

    # Detailed misclassification breakdown
    mis = df[df['predicted_class'] != df['class_CWE']]
    if mis.empty:
        print("\nNo misclassifications. All samples routed correctly.")
    else:
        counts = (
            mis
            .groupby(['class_CWE', 'predicted_class'])
            .size()
            .reset_index(name='count')
            .sort_values(['class_CWE', 'count'], ascending=[True, False])
        )
    print("\n=== Misclassification Breakdown ===")
    print(counts.to_string(index=False))

    # Full confusion matrix (not normalized)
    conf_matrix = (
        df.groupby(['class_CWE', 'predicted_class'])
        .size()

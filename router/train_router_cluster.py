...
def trainer(train_pd, val_pd, test_pd, args):
    ...
    weights = []
    for type in train_pd['new_class_CWE'].value_counts().keys():
        class_data = train_pd[train_pd.new_class_CWE == type]
    ...
    if os.path.exists(model_path):
        ...
        if not args.ros:
            ...
            train_pd, y_resampled = ros.fit_resample(train_pd, train_pd['new_class_CWE'])
            train_pd = train_pd.reset_index(drop=True)
            print(train_pd.new_class_CWE.value_counts().to_dict())
        
        # Initialize predictor
        predictor = MultiModalPredictor(
            label='new_class_CWE', path=model_path,eval_metric ='f1_macro'
        )
        predictor.fit(
        train_data=train_pd,
        tuning_data=val_pd,
    ...
    if inference_test:
        ...
        test_dataset = test_pd[["processed_func", "new_class_CWE"]]
    ...
    if inference_train:
        ...
        train_dataset = train_pd[["processed_func", "new_class_CWE"]]
    ...
    if inference_val:
        ...
        val_dataset = val_pd[["processed_func", "new_class_CWE"]]
...
def parse_args():
    ...
    parser.add_argument(
        "--train_file",
        type=str,
        default="datasets/new_j_processed_train.csv",
        help="train file",
    )
    parser.add_argument(
        "--test_file",
        type=str,
        default="datasets/new_j_processed_test.csv",
        help="test file",
    )
    parser.add_argument(
        "--val_file",
        type=str,
        default="datasets/new_j_processed_val.csv",
        help="val file",
    )
...
def main():
    ...
    lv1_cwes = find_cwes(train_pd, 'new_class_CWE')
    
    # Filter datasets to include only known CWEs
    train_lv1 = train_pd[train_pd['new_class_CWE'].isin(lv1_cwes)]
    val_lv1 = val_pd[val_pd['new_class_CWE'].isin(lv1_cwes)]
    test_lv1 = test_pd[test_pd['new_class_CWE'].isin(lv1_cwes)]
    

    train_pd = train_lv1[["processed_func", "new_class_CWE"]]
    val_pd = val_lv1[["processed_func", "new_class_CWE"]]
    test_pd = test_lv1[["processed_func", "new_class_CWE"]]
